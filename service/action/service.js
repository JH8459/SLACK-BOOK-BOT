...
exports.CreateBookListModalByGenre = async (genre, page) => {
  // 노션에서 장르별 도서 리스트를 가져오는 요청
  const result = await NotionBookListGroupByGenre(genre);
  if (result.bookList.length >= page) {
    // 노션에서 가져온 데이터를 슬랙 블럭(도서 블럭) 형태로 제작한다
    const bookListBox = result.bookList[page - 1]
      .map((book) => {
        // Box를 만들어주는 함수
    ...
    const completeBookListModal = CreateCompleteBookListModal(
      genre,
      bookListBox,
      result.total,
      page,
...
exports.CreateReplyListModalByTitle = async (genre) => {
  // 노션에서 장르별 도서 리스트를 가져오는 요청
